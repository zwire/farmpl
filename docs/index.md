...
結果ダッシュボードにある「CSVをダウンロード」ボタンから、計画の内容をまとめたファイルを保存できます。
ファイルはZIP形式で、中には次の表（CSV）が入っています。どれもExcelやスプレッドシートで開けます。

- **合計の表（totals.csv）**
  - 全体の売上と労務費、最も忙しい時期の作業時間がわかります。

- **作物ごとの表（crops.csv）**
  - 作物名、面積、単価、売上、作業時間、労務費の一覧です。

- **作業イベントの表（gantt_events.csv）**
  - いつ・どの作物の・どんな作業をするか、関係する圃場とその作業時間の合計を一覧にしています。

- **作業者×時期の表（worker_hours_by_period.csv）**
  - 作業者ごとに、時期（例：10月上旬/中旬/下旬）ごとの作業時間をまと めています。名前と役割も一緒に出ます。

ボタンが押せないときは、まだ計算が終わっていないか、接続設定が完了していない可能性があります。計算が終わると自動で押せるようになります。
