...
以下は、docs仕様（model.md / tech.md / requirement.md）を実装へ反映し、最小→完成へ到達するための実行用タスクリスト。

各タスクは完了時に [x] へ更新。制約は「実装・テスト・サンプルデータ更新」をセットで行う。
...
- [x] 実装: `model.md` の占有状態、面積恒常性を利用して x[l,c,t] を一定化（active 区間）
- [x] 実装: active 区間外では 0 へ遷移可（ブロック日や次作への切替）
- [x] テスト: active 区間で一定、区間境界でのみ変化
- [x] サンプル更新: イベント（播種→収穫）を設定し挙動確認
...
3.3 労働時間最小化
- [x] 実装: Σ h[w,e,t] を最小化
- [x] テスト: 労働負荷の小さい解が選ばれる
- [x] サンプル更新: 効果がわかるようにサンプル更新

3.4 土地遊休（Idle）最小化（新規）
- [x] 実装: Σ idle[l,t] を最小化
- [x] テスト: idle が減る方向の解が選ばれる
- [x] サンプル更新: 効果がわかるようにサンプル更新

3.5 多品目志向（Diversity）または単一志向
- [x] 実装: use_c（二値）導入し項を追加（最大/最小）
- [x] テスト: 多様性重みで解が変わること
- [x] サンプル更新: 効果がわかるようにサンプル更新
