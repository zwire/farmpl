...
- [ ] 5. APIクライアント層を作成する
  - Files: ui/lib/api/client.ts, ui/lib/api/endpoints.ts
  - 内容: `/v1/optimize`、`/v1/optimize/async`、`/v1/jobs/{id}` GET/DELETE向けの型付きfetchヘルパーを実装し、APIキー付与・リトライ・タイムアウト・キャンセルに対応する。
  - 目的: HTTPロジックを集約し、UIから容易に呼び出せる関数/フックを提供する。
  - _Leverage: lib/services/optimizer_adapter.py (挙動参照), ui/lib/types/planning.ts_
  - _Requirements: 2, Non-Functional (Security, Reliability)
  - _Prompt: Implement the task for spec farmpl-planning-ui, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Frontend networking engineer | Task: Author a resilient API client with retry/backoff, auth header injection, and abort support per requirement 2 and security/reliability NFRs | Restrictions: No global fetch monkeypatching, log errors via provided hook, expose promise-based functions and React Query adapters | _Leverage: browser fetch, AbortController, ui/lib/types/planning.ts | _Requirements: 2, NF-Security, NF-Reliability | Success: Client handles success/error paths, unit tests (later) can stub fetch, headers configurable without leaking secrets_

- [ ] 6. 送信とジョブ監視パネルを開発する
  - Files: ui/app/(planning)/components/submission-panel.tsx, ui/app/(planning)/components/job-monitor.tsx
  - 内容: 同期/非同期切替、Idempotency Key生成、APIキー入力、ポーリング、キャンセルを扱うReactコンポーネントを作る。
  - 目的: 要件2で求められるリクエスト送信と進捗把握をUIで実現する。
  - _Leverage: ui/lib/api/client.ts, ui/lib/state/planning-store.ts, ui/lib/types/planning.ts_
  - _Requirements: 2
  - _Prompt: Implement the task for spec farmpl-planning-ui, first run spec-workflow-guide to get the workflow guide then implement the task: Role: React application engineer | Task: Implement submission and job monitoring components using the API client and state store per requirement 2 | Restrictions: Keep polling interval configurable, ensure cleanup on unmount, provide accessible buttons and status text | _Leverage: React Query (add dependency), ui/lib/api/client.ts | _Requirements: 2 | Success: Users can submit sync/async requests, see status updates, cancel jobs; errors surfaced with actionable guidance_

- [ ] 7. 結果ダッシュボードコンポーネントを構築する
