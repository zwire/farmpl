...
import { EventGraphEditor } from "../events/EventGraphEditor";
import {
  ComboBox,
  MultiComboBox,
  type ComboBoxOption,
...
function EventsSection({
  ...
  const handleUpdate = (index: number, patch: Partial<PlanFormEvent>) => {
    onPlanChange((prev) => {
  ...
  const applyLabor = (
    index: number,
    patch: Partial<NonNullable<PlanFormEvent["labor"]>>,
  ) => {
    const current = plan.events[index].labor ?? {};
    const next: NonNullable<PlanFormEvent["labor"]> = { ...current };
    ...
    handleUpdate(index, { labor: cleaned });
  };

  const applyLag = (
    index: number,
    patch: Partial<NonNullable<PlanFormEvent["lag"]>>,
  ) => {
    const current = plan.events[index].lag ?? {};
    const next: NonNullable<PlanFormEvent["lag"]> = { ...current };
    for (const [key, val] of Object.entries(patch)) {
      if (val === undefined) {
        delete (next as Record<string, unknown>)[key];
      } else {
    ...
    const cleaned = Object.keys(next).length === 0 ? undefined : next;
    handleUpdate(index, { lag: cleaned });
  };
  ...
  const handleAdd = () => {
    onPlanChange((prev) => ({
      ...
      events: [
        ...prev.events,
        {
          id: createUniqueId("event", prev.events.map((event) => event.id)),
          cropId: prev.crops[0]?.id ?? "",
          name: "",
          category: "",
          usesLand: true,
  ...
  const handleDependencyChange = (
    ...
    const targetIndex = plan.events.findIndex((event) => event.id === targetId);
    if (targetIndex === -1) return;
    handleUpdate(targetIndex, { precedingEventId: sourceId ?? undefined });
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <SectionCard
        title="イベント依存関係"
        description="イベント同士の依存関係をノードとエッジで編集します"
      >
        <EventGraphEditor
          events={plan.events}
      ...
      <SectionCard
        title="イベント詳細"
        description="各イベントの属性やリソースを設定します"
        actionLabel="イベントを追加"
        onAction={handleAdd}
        emptyMessage="イベントが登録されていません。"
          ...
          >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Field label="対象作物">
                <ComboBox
                  value={event.cropId}
                  onChange={(next) => handleUpdate(index, { cropId: next })}
                  options={cropOptions}
                  disabled={cropOptions.length === 0}
                  placeholder={
                    cropOptions.length === 0
              ...
              <Field label="土地を使用">
                <select
                  value={String(event.usesLand)}
                  onChange={(e) =>
                    handleUpdate(index, { usesLand: e.target.value === "true" })
                  }
                  className="rounded-md border border-slate-300 px-2 py-2 text-sm"
                >
              ...
              <Field label="必要人数">
                <input
                  type="number"
                  min={0}
                  value={event.labor?.peopleRequired ?? ""}
                  onChange={(evt) =>
                    applyLabor(index, {
                      peopleRequired:
                        evt.target.value === ""
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Field label="開始条件 (日)">
                <input
                  value={(event.startCond ?? []).join(",")}
                  onChange={(evt) =>
                    handleUpdate(index, {
                      startCond: toNumberList(evt.target.value),
                    })
                  }
                  className="rounded-md border border-slate-300 px-3 py-2 text-sm"
                />
              ...
              <Field label="終了条件 (日)">
                <input
                  value={(event.endCond ?? []).join(",")}
                  onChange={(evt) =>
                    handleUpdate(index, {
                      endCond: toNumberList(evt.target.value),
                    })
                  }
                  className="rounded-md border border-slate-300 px-3 py-2 text-sm"
                />
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Field label="必要ロール">
                <MultiComboBox
                  value={event.requiredRoles ?? []}
                  onChange={(selected) =>
                    handleUpdate(index, {
                      requiredRoles: selected.length > 0 ? selected : undefined,
                    })
                  }
                  options={roleOptions}
                ...
                {roleOptions.length === 0 && (
                  <p className="text-[11px] text-slate-400">
                    作業者にロールを設定すると選択できるようになります。
                  </p>
                )}
              </Field>
              <Field label="必要リソース">
                <MultiComboBox
                  value={event.requiredResources ?? []}
                  onChange={(selected) =>
                    handleUpdate(index, {
                      requiredResources:
...
function EntityCard({ title, id, children, onRemove }: EntityCardProps) {
  return (
...
function MeasurementInput({ measurement, onChange }: MeasurementInputProps) {
...
const createUniqueId = (prefix: string, existing: Iterable<string>) => {
