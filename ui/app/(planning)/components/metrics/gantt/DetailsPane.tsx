...
export const DetailsPane = ({
  ...
  if (!item) {
    return (
      <div className="flex h-full min-h-[300px] flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50/80 p-6 text-center text-sm text-slate-500">
    ...
    <div className="flex h-full flex-col">
      ...
      <ul className="mt-3 flex-1 space-y-3 overflow-y-auto pr-2">
        {events.map((ev) => (
          <li
            key={ev.id}
            className="rounded-md border border-slate-200 bg-slate-50/80 p-3 text-sm"
          >
            <div className="flex items-center gap-2">
              <div className="font-semibold text-slate-700">{ev.label}</div>
            </div>
            ...
            {ev.workerUsages.length > 0 && (
              <div className="mt-2">
                <div className="text-xs font-medium text-slate-600">作業者</div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-slate-200 pb-3">
        ...
        <p className="text-sm text-slate-500">期間: {periodLabel}</p>
      </div>
      ...
      <ul className="flex-1 space-y-2 overflow-y-auto pr-2 text-xs">
        {items.map((item) => (
          <li
            key={item.name}
            className="flex items-center justify-between rounded-md p-2 transition-colors hover:bg-slate-50"
          >
...
const ProgressBar = ({
  ...
  return (
    <div className={`mt-1 h-1 w-24 rounded-full bg-slate-200 ${className}`}>
      <div
